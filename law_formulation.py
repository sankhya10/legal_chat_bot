...
def outline_guided(keywords,query):
    #outline_model = models.transformers("gpt2")
    ...
    # answer = generate.choice(outline_model,["Relevant","Not Relevant"])(prompt)
    messages = [SystemMessage(content=prompt),HumanMessage(content="Answer either relevant or not relevant")]
...
def refine_keywords(lawyer_keyword,query):
    ...
    # context = f"You are an expert in extracting values from a dictionary based on the key and a query \
    #             Your only job is to extract value from this {lawyer_keyword} dictionary where the key of the dictionary is already decided as {keyword}\
    #             The extracted value should be relevant to this query: {query} and length of the value extracted has to be at most TWO!! \
    #             The output format would be:{keyword}:(your_recommended_keyword) If you can't find any value relevant to the {query} just emit not available"
       ...
       # print("extracted_dict",extracted_dictionary)
        if isinstance(extracted_dictionary ,set):
            ...
            if len(extracted_dictionary.values()) == 1:
                return list(extracted_dictionary.keys())[0],list(extracted_dictionary.values())[0][0]
            else:
...
def main():
    ...
    #print(outline_guided(law_dict,query))

   # print(law_dict['business-law'])
    while True:
        ...
        if "not relevant" in refine:
            ...
            if matches:
                if "business" in matches[0]:
                    extracted_answer = refine_keywords(law_dict['business-law'],query).content
                    search_dict_pattern(extracted_answer,query)            
                if "personal" in matches[0]:
                    extracted_answer = refine_keywords(law_dict['personal-law'],query).content
